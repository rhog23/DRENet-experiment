...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            # Augment imagespace
            if not mosaic:
                img, dgimg, labels = random_perspective(
                    img,
                    dgimg,
                    labels,
                    degrees=hyp["degrees"],
            ...
            # Augment colorspace
            augment_hsv(
                img, dgimg, hgain=hyp["hsv_h"], sgain=hyp["hsv_s"], vgain=hyp["hsv_v"]
            )
    ...
    def collate_fn4(batch):
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                ...
                dgim = torch.cat(
                    (
                        torch.cat((dgimg[i], dgimg[i + 1]), 1),
                        torch.cat((dgimg[i + 2], dgimg[i + 3]), 1),
                    ),
                    2,
                )
                l = (
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        return (
            ...
            (h0, w0),
            img.shape[:2],
        )  # img, dgimg, hw_original, hw_resized
    else:
        return (
            self.imgs[index],
            self.img_hw0[index],
...
def augment_hsv(img, dgimg, hgain=0.5, sgain=0.5, vgain=0.5):
    ...
    dgimg_hsv = cv2.merge(
        (cv2.LUT(dhue, lut_hue), cv2.LUT(dsat, lut_sat), cv2.LUT(dval, lut_val))
    ).astype(dtype)
    if img.shape[2] != 3:
...
def load_mosaic(self, index):
    ...
    # Augment
    img4, dgimg4, labels4 = random_perspective(
        img4,
        dgimg4,
        labels4,
...
def letterbox(
    img, new_shape=(640, 640), color=(0, 0, 0), auto=True, scaleFill=False, scaleup=True
):
    ...
    if img.shape[2] != 3:
        ...
        img = cv2.copyMakeBorder(
            img[:, :, :3], top, bottom, left, right, cv2.BORDER_CONSTANT, value=color
        )  # add border
...
def random_perspective(
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            dgimg = cv2.warpPerspective(
                dgimg, M, dsize=(width, height), borderValue=(114, 114, 114)
            )
        else:  # affine
            img = cv2.warpAffine(
                img, M[:2], dsize=(width, height), borderValue=(114, 114, 114)
            )
            dgimg = cv2.warpAffine(
                dgimg, M[:2], dsize=(width, height), borderValue=(114, 114, 114)
            )
